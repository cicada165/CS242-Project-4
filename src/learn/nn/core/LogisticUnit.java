...
public class LogisticUnit extends NeuronUnit {
	...
	public void update(double[] x, double y, double alpha) {
		...
		double w0 = this.getWeight(0);
		w0 = w0 + alpha * (y - t) * t * (1 - t);
		this.setWeight(0, w0);
			
		//Update weight for each connection using the logistic regression gradient descent rule
		for(int i = 0; i < x.length; i++) {
			double w = this.getWeight(i+1);
			w = w + alpha * (y - t) * t * (1 - t) * x[i];
