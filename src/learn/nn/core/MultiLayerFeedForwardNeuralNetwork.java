...
abstract public class MultiLayerFeedForwardNeuralNetwork extends FeedForwardNeuralNetwork {
	 ...
	 */
	public MultiLayerFeedForwardNeuralNetwork(int ninputs, int[] nhiddens, int noutputs) { //This is the method to 
		this(new Unit[nhiddens.length+2][]);
		// Input layer
		this.layers[0] = new InputUnit[ninputs];
		for (int i=0; i < ninputs; i++) {
			this.layers[0][i] = new InputUnit(); //The first layer is the input layer and is filled with input units
		}
		// Hidden layers: each connected to all previous layer units
		for (int l=1; l <= nhiddens.length; l++) {
			int n = nhiddens[l-1]; 
			this.layers[l] = new LogisticUnit[n];
			for (int j=0; j < n; j++) { //All the other layers except the last layer are hidden layers
				this.layers[l][j] = new LogisticUnit(); //Filled with logistics unit(NeuronUnit)
				for (int i=0; i < layers[l-1].length; i++) {
					new Connection(this.layers[l-1][i], this.layers[l][j]); //Construct the connection to every unit in the previous layer for every unit in this layer
				}
			}
		}
	 ...
	 */
	public void initializeWeights() { //Initialize all the weights
		for (int l=1; l < this.layers.length; l++) {
			NeuronUnit units[] = (NeuronUnit[])this.layers[l];
			for (int i=0; i < units.length; i++) {
				NeuronUnit unit = units[i];
	...
	public void backprop(Example example, double alpha) {
		...
		for(int j = 0; j < this.layers[l].length; j++) {
			...
			LogisticUnit lu = (LogisticUnit)this.layers[l][j];
			
			double in_j = lu.getInputSum();
			double y_j = example.outputs[j];
			double a_j = lu.activation(in_j);
			
			lu.delta = lu.activationPrime(in_j) * ( y_j - a_j);
			}
		...
		for(int m = l - 1;m > 0;m--) {
			...
			for(int n = 0; n < this.layers[m].length; n++) {
				...
				LogisticUnit lu = (LogisticUnit)this.layers[m][n];
				double in_i = lu.getInputSum();
				//sum_j w_ij * Delta[j]
				double s = 0;
				
				for(int t = 0; t < lu.outgoingConnections.size(); t++) {
					LogisticUnit lu2 = (LogisticUnit)this.layers[m+1][t];
					s = s + lu.outgoingConnections.get(t).weight * lu2.delta;
					}
				
				lu.delta = lu.activationPrime(in_i) * s;
			}
		}
		...
		for(int k = 1; k < this.layers.length;k++) {
			...
			for(int j = 0; j < this.layers[k].length; j++) {

				LogisticUnit lu3 = (LogisticUnit)this.layers[k][j];
				
				//Find all the connections 
				for(int i = 0; i < this.layers[k-1].length;i++) {
					
					//w_ij <- w_ij + alpha * a_i * delta_j
					if(i == 0) {
						lu3.incomingConnections.get(i).weight += alpha * lu3.delta;
					}else {
						lu3.incomingConnections.get(i).weight += alpha * lu3.delta * this.layers[k-1][i-1].getOutput();
					}
